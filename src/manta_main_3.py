...
import arrowsPressedReader as arp



## this is supossed to be run to move Manta with the keyboard,
## then the data will be used to pretrain the DNN models
...
def signal_handler(signal, frame):
	print('You pressed ctrl C. Stopping the simulation...\r')
	dnn.save()
...
arrowsListener = arp.ActionsKeywordBuffers()
arrowsListenerThread = arp._ArrowsReaderThread(arrowsListener)
...
for retrain in range(nb_dnn_retrains): # until convergence in reward, o otro criterio de parada
	...
	for sim in range(nb_sims):
		...
		if crashed == False:
			...
			print("REWARD (not crashed)", reward, "\r")
			...
			print("REWARD (not crashed part)", reward, "\r")
			if ((sum(reward)/len(reward)) if len(reward) else 0) > 1:
		...
		return_code = vrep.simxStopSimulation(clientID,vrep.simx_opmode_oneshot)
		time.sleep(1)
		print("simulation", sim + 1, "out of", nb_sims, "finished.\r")
		#vrep.simxFinish(clientID)

	print("Vamos a reentrenar la red con", len(x_training_labels), "ejemplos nuevos...\r")
